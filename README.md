...
* 各省储行创立发行的数字五民币发行后即汇入该省储行质押钱包，并永久质押；    
## 3.3.省储行质押发行
...
# 4.费用及利息
## 4.1.链上交易费
* 链上交易费由链上交易手续费和链上交易小费组成，链上交易手续费为0.1%，由付款方必须支付，链上交易小费则由付款方在支付发起时自行设定小费金额，默认为0元/笔，可选择1元及任意整倍数/笔交易；  
* 链上交易费的90%用于奖励铸块的全节点，10%支付给国储会权威节点交易钱包用于社区建设和权威节点运营，链下交易和纸质五民币的交易手续费不适用于此条规则；  
* 链下交易上链的，除省储行权益节点获得的手续费外，其他链下交易不用再支付链上交易费，例：小李支付1000元五民币给小习，由广东省储行权益节点清算的，费率为0.1%，那么小李需要支付1001元，小习收到1000元，广东省储行权益节点收到1元，该交易上链时，那1000元不用支付链上交易费，而广东省储行获得的1元需要支付链上交易手续费。  
## 4.2.链下交易费
* 链下交易手续费由各省储行权益节点自行设定，可设定的范围为0.01%至0.1%，且单笔支付手续费最少为1分，不足1分的以1分计算，链下交易费全部用于奖励各省储行权益节点；  
...
* 国储会节点ID：nrc_poa_node，与交易钱包地址和节点PeerID绑定；
* 门限签名：N=19，T≥13
* 使用专用的桌面前端：国储会权威节点软件/dt_nrc_node，系统：Linux；
#### 6.1.3.2.省储会节点/prc_poa_nodes
* 省储会节点ID：prc_poa_nodes_0001至prc_poa_nodes_0043，每个省储会节点ID绑定一个交易钱包地址和一个节点PeerID；
* 门限签名：N=9，T≥6
* 使用专用的桌面前端：省储会权威节点软件/dt_prc_node，系统：Linux；
#### 6.1.3.3.省储行节点/prb_pos_nodes
* 省储行节点ID：prb_pos_nodes_0001至prb_pos_nodes_0043，每个省储行节点ID绑定一个交易钱包地址、一个质押钱包地址和一个节点PeerID；
* 门限签名：N=9，T≥6
* 使用专用的桌面前端：省储行权益节点软件/dt_prb_node，系统：Linux；
...
* 使用通用的公民轻节点手机软件，即WuminApp手机App，含iOS和Android端，WuminApp是一个集成钱包、投票、视频、匿名社交的软件，仅能进行链下交易；
...
* 单一地址钱包：国储会权威节点软件（dt_nrc_node）、省储会权威节点软件（dt_prc_node）、公民轻节点软件（WuminApp），这些节点使用的节点软件只能生成一个交易钱包地址；
#### 6.1.5.2省储行钱包/prb_wallet
* 双地址钱包：省储行权益节点软件（dt_prb_node），省储行节点软件仅能生成一个交易钱包地址和一个质押钱包地址;
