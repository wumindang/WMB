...
* 五民币区块链是一个结合公权、金融、市场和区块链的综合系统，将民主化运动和建立联邦制共和国与去中心化的区块链系统深度绑定，以此构建一个具有公信、安全、开放、透明、高效的主权区块链系统，实现跨国界的链上链下实时收付。
...
## 2.1.节点概览
   ...
   |  功能  | 国家铸币权、监管 | 省铸币权、监管 |  链下交易验证  |    PoW铸块    |  监督、链下支付  |  链上链下支付 |
...
* 国储会权威节点软件拥有发起投票、发行数字五民币、销毁数字五民币、增删改新省储会、增删改新省储行、部署公民轻节点发行智能合约和交易钱包收入与支出等功能，并承担数据归档和引导节点；
* 国储会节点拥有发起发行数字五民币、销毁数字五民币、增删改新省储会、增删改新省储行、部署公民轻节点发行智能合约流程的，需权威节点间投票（国储会、省储会）,且公民轻节点可投票驳回。
## 2.3.省储会权威节点  
...
* WuminAppID（公民轻节点PeerID）、交易钱包地址和公民身份识别码一对一绑定，完成绑定后可获得投票。  
## 2.7.访客轻节点
