...
## 1.3.发行量概览
   ...
   |   奖励发行       |   5,773,989,512.00元 = 57.73亿      |    奖励发行智能合约   | 发行即存入奖励智能合约      |
   |   省储行创立发行  |   144,349,737,800.00元 = 1443.49亿  |       省储行        | 发行即存入质押钱包         |
   |   省储行质押发行  |   36,809,183,139.00元 = 368.09亿    |   质押发行智能合约    | 发行即存入质押智能合约      |
   |   全节点发行     |   9,999,899,000.01元 = 99.99亿      |        全节点        | 写入PoW共识              |
   |   轻节点发行     |   1,571,981,633,622.00元 = 1.57万亿  |   轻节点发行智能合约   | 发行即存入公民认证智能合约  |
...
## 2.2.超级节点
* 有1个超级节点，不能增删改，负责初始化系统，并生成创世区块，在完成区块链初始化后，保留增删改新省储会、增删改新省储行、升级逻辑功能智能合约和交易钱包收入与支出权限；
...
# 3.发行与销毁
## 3.1.创世发行
* 创世发行1,443,497,378.00元数字五民币（14.43亿元），由创世区块发行，创始团队所有,创世发行量以中共第7次人口普查数据为基准，每一个中共国人发行1.00元；
   |    中共第7次人口普查数据   |          创世发行量       |    持有地址(超级节点交易钱包地址)      |
   |:-----------------------:|:-----------------------:|:-----------:|
   |    1,443,497,378人      |   1,443,497,378.00元     |   xxxxxx   |   
* 一个人在社会中享有若干权益的同时，同样应尽若干义务，尽了若干义务的，亦应享受若干权益，创世发行寓意每一个中共国人支付1.00元，以支持五民主义和去中心化民主运动。  
## 3.2.融资发行
* 融资发行1,443,497,378.00元数字五民币（14.43亿元），融资购买价格为每1.00元数字五民币0.001USDT，即1USDT=1000.00元数字五民币；单笔购买金额超过1,000,000.00元数字五民币的，在五民币区块链上线后将获得1:4的空投奖励，单笔购买少于1.000,000.00元的不计入奖励资格，预计融资额1,443,497.378USDT（144.34万USDT），由国储会权威节点发行；
* 国储会地址
   |           国储会交易钱包地址         |                国储会USDT钱包地址             | 
   |:---------------------------------:|:-------------------------------------------:|
   |  xxxxxxxxxx                       |  0x6c9c46D9c9aA4411422D92401D8A7445645CD1f4 | 
* 融资用途
   |          项目                            |     占比    |  预算金额（USDT）  |
   |:--------------------------------------------:|:-----------:|:---------------:|
   |中华民族联邦共和国国家官方网站的开发               |  0.69%      |     1.0万        |
   |中国五民党官方网站的开发                         |  0.69%      |     1.0万        |
   |五民币金融区块链系统的开发                       |  13.86%     |     20.00万      |
   |联邦公民身份识别码系统的开发                     |   3.46%     |     5.00万       |  
   |联邦公民护照管理系统的开发                       |   3.46%     |     5.00万       |
   |WuminApp匿名社交软件的开发（含公民轻节点钱包功能）   |   6.93%     |     10.00万      |
   |市场推广与社区建设                              |   13.86%    |    20.00万      |
   |交易所及流动性相关费用                          |   20.78%     |    30.00万      |
   |法律、管理及其他费用                            |   6.93%     |     10.00万     |
   |五民党安全基金                                 |   29.34%    |    42.35万      | 

* 购买融资发行五民币单笔购买金额超过100万元数字五民币的用户，将获得空投奖励，空投奖励为1:4;
   |     类型    |       奖励金额      |  奖励方式    |              奖励要求                  |
   |:----------:|:--------------:|:-----------:|:----------------------------------:|
   |  融资发行   |   每1元奖励4元   |   奖励发行   |   每笔购买100万元五民币及以上    |
   |  其他发行   |      无奖励     |       -     |                  -                 |
...
* 奖励发行最大总量=融资发行量x4，如下所示：
   |    类型   |       发行量         |      简述     |
   |:--------:|:-------------------:|:-------------:|
   |  融资发行 |  1,443,497,378.00元  |   14.43亿元   | 
   |  奖励发行 |  5,773,989,512.00元  |   57.73亿元   |
## 3.4.省储行创立发行
...
* 创世配置文件使用json定义区块链的创世状态，包括区块链名称、ID、网络标识、引导节点、创世状态、节点映射表等信息，其中，创世状态定义创世宣言、创世发行、融资发行、创世合约，以及持有创世发行五民币的各账户地址和金额。
### 6.1.2.共识/consensus
#### 6.1.2.1.工作量证明/PoW
...
#### 6.1.2.3.权威证明/PoA
* 含超级节点、国储会权威节点、省储会权威节点，超级节点负责区块链初始化、升级逻辑功能智能合约、增删改新省储会/新省储行，国储会权威节点负责发行和销毁，省储会权威节点参与发行和销毁；
### 6.1.3.节点/nodes
#### 6.1.3.1.超级节点/super_node
* 超级节点ID：super_node_1,与交易钱包地址和PeerID绑定；
* 门限签名：N=5，T≥3
* 使用专用的超级节点软件，桌面端：Linux；
...
* 合约地址：super_node_1_5ContractBonus... 
* 持有金额：577398951200分；
...
* 逻辑功能智能合约只能由超级节点部署和升级；
* 逻辑智能合约是可以升级的智能合约，负责奖励发行智能合约和公民轻节点发行智能合约的逻辑功能的具体实现；
* 奖励发行智能合约和公民轻节点发行智能合约的相关功能在此逻辑功能智能合约中实现，根据具体规则和要求区块链上线后升级；
### 6.7.2.去中心化应用/dapp；
## 6.8.桌面前端/desktop
* 所有桌面前端节点软件均使用Linux系统；
### 6.8.1.超级节点/dt_super_node
* 导入钱包，显示交易钱包余额、节点ID，发送和接收五民币，部署创世智能合约和升级逻辑功能智能合约；
