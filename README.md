...
## 1.2.名称与简介  
* 名称：五民币区块链（英文：Wuminbi Blockchain，简称：WBC），原生代币：五民币，符号：WMB；
* 单位：常用单位：元（YUAN），小数单位：分（FEN），元最小为1元，分最小为1分，100分等于1元，系统统一使用分为计算单位，前端统一使用元为展示单位；
...
## 1.3.发行量概览
   |    发行类型      |   最大发行金额            |         简述       |      发行方     |
   |:---------------:|:-------------------------:|:---------------:|:---------------:|
   |   创世发行       |   1,443,497,378.00元        |   14.43亿      |     超级节点     |
   |   融资发行       |   1,443,497,378.00元        |   14.43亿      |      国储会      |
   |   奖励发行       |   2,886,994,756.00元        |   28.86亿      |  奖励发行智能合约  |
   |   省储行创立发行  |   144,349,737,800.00元      |   1443.49亿    |      省储行      |
   |   省储行质押发行  |   36,809,183,139.00元       |   368.09亿     |  质押发行智能合约  |
   |   全节点发行     |   9,999,899,000.01元        |    99.99亿     |       全节点      |
   |   轻节点发行     |   1,571,981,633,622.00元    |   1.57万亿     |  轻节点发行智能合约  |
   |   决议发行       |           N                |       -         |      国储会      |
   | **预计总发行量**  |  **1,768,914,443,073.01元** |  **1.76万亿**   |        -        |
...
* 所有获得公民轻节点认证发行的奖励，均处于锁定状态，在持有该轻节点的公民完成《公民素质教育》题库学习后，即可解锁奖励资金，公民轻节点发行智能合约自动发放奖励；  
...
****
# 6.技术开发
* 后端主要语言rust，采用Substrate框架，智能合约lnk，桌面前端Tauri，手机前端Flutter
## 6.1.核心模块/core
### 6.1.1.区块链/block
