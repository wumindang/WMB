...
// 中文注释：启动国储会节点命令，前端通过 Tauri 调用此函数
#[tauri::command]
async fn start_node() {
    println!("中文注释：国储会节点启动");  // 中文注释：在终端打印启动信息
    // 中文注释：实际节点逻辑待实现，例如初始化网络连接
}

// 中文注释：获取国储会节点状态命令，前端通过 Tauri 调用此函数
#[tauri::command]
async fn get_status() -> usize {
    let interface = Interface::new("nrc_node.dat", "127.0.0.1:8081");  // 中文注释：初始化接口实例，连接存储和网络
